# SERIES 3 - Hosted Checkout with Payment Sessions (Laravel 13)
...
> This project is the working reference implementation: raw cURL, no SDK.
...
cp .env.example .env        # set SNIPPE_API_KEY and APP_URL (https)
php artisan serve           # http://localhost:8000
```

Optional base URL overrides (defaults are correct for most accounts - see
`config/snippe.php`):
...
1. Open the store at http://localhost:8000 and register an account. The form
...
6. After paying you return to your site at `/order/success/{reference}`. Notice
   the order status: it is `pending`. The payment confirmation arrives
   separately (webhook) - see the status flip to `completed` on refresh.

## Key Integration Files (in this repo)

| # | File | What it does |
|---|---|---|
| 1 | `app/Http/Controllers/CheckoutController.php` - `processPay()` | Creates the session from the cart + logged-in user, redirects to `checkout_url` |
| 2 | `app/Services/SnippeApiService.php` - `createSession()` | Raw cURL call to `POST /api/v1/sessions` (headers, idempotency key) |
| 3 | `app/Http/Controllers/WebhookController.php` - `handle()` | Receives the async status update, reconciles by `metadata.order_reference` |
| 4 | `resources/views/auth/register.blade.php` | Collects the phone number used to pre-fill the hosted checkout |

There are **two Snippe API bases** (both configured in `config/snippe.php`):
...
| `redirect_url` | `APP_URL/order/success/{reference}` | Where the customer lands after paying |
...
`/order/success/{reference}`, and the order status tells the story. That is fine
for a demo; in production you may want a distinct page for cancelled sessions.
The important part: the route exists on your server, so the customer always
comes back to your site.

Check your routes any time you touch the return URL:
...
(`session()->forget('cart')`). One checkout intent should produce one session.
Do not re-create sessions on every retry or page reload - repeated session
...
The `/v1` payments methods (`createMobilePayment()`, `createCardPayment()`,
`createQrPayment()` in `app/Services/SnippeApiService.php`) power the custom
checkout form flow - currently disabled, kept for later.
